...
std::string MarkdownConverter::convertToLatex(const std::string &markdown)
{
    ...
    while (std::getline(stream, line))
    {
        ...
        if (line.find("```") == 0)
        {
            ...
            {
                inCodeBlock = false;
                result << "\\begin{lstlisting}[language=" << (codeBlockLanguage.empty() ? "text" : codeBlockLanguage) << "]\n";
...
std::string MarkdownConverter::convertImages(const std::string &line)
{
    ...
    result = std::regex_replace(result, imagePattern, "\\\\begin{figure}\n\\\\centering\n\\\\includegraphics{$2}\n\\\\caption{$1}\n\\\\end{figure}");
...
std::string MarkdownConverter::convertCitations(const std::string &line)
{
    ...
    while (std::regex_search(searchStart, result.cend(), matches, citationPattern))
    {
        ...
        result.replace(matches.position() + (searchStart - result.cbegin()),
                       matches.length(), citation);
...
std::string MarkdownConverter::generateBibTeX()
{
    ...
    for (const auto &ref : citationRefs)
    {
        ...
        //std::cout << "Found " << papers.size() << " papers." << std::endl;

        if (!papers.empty()) {
            for (size_t i = 0; i < papers.size(); ++i) {
                const auto& paper = papers[i];
                std::cout << i + 1 << ". " << paper.title << " (" << paper.year << ")" << std::endl;
            ...
            std::cout << "Please select a paper, input the number" << std::endl;
            ...
            //std::cout << "Saving all results to references.bib" << std::endl;
...
//{
//    // First, we need to identify and protect LaTeX commands that are already present
//    // This is a more sophisticated approach than the previous one
//
//    // Step 1: Make a copy of the input text
//    std::string result = text;
//
//    // Step 2: Find and temporarily mark LaTeX commands to protect them
//    std::regex latexCmdPattern("\\\\[a-zA-Z]+(\\{[^\\}]*\\})*");
//    std::string tempText = result;
//    std::smatch matches;
//    std::vector<std::string> commands;
//
//    // Find all LaTeX commands and replace them with placeholders
//    while (std::regex_search(tempText, matches, latexCmdPattern))
//    {
//        std::string cmd = matches[0].str();
//        std::string placeholder = "##LATEXCMD" + std::to_string(commands.size()) + "##";
...
//        {
//            result.replace(pos, cmd.length(), placeholder);
//        }
//
//        // Move to the next match
//        tempText = matches.suffix();
//    }
//
...
//
//    // Step 4: Restore LaTeX commands
//    for (size_t i = 0; i < commands.size(); ++i)
//    {
//        std::string placeholder = "##LATEXCMD" + std::to_string(i) + "##";
//        size_t pos = result.find(placeholder);
//        if (pos != std::string::npos)
//        {
//            result.replace(pos, placeholder.length(), commands[i]);
...
//}
 ...
 std::string MarkdownConverter::escapeLatexChars(const std::string& text) {
    ...
    // But we need to be careful not to escape characters that are already part of a LaTeX command
     ...
     // This is a simplified approach - in a real implementation, this would need to be more sophisticated
     std::regex escapePattern("([\\#\\$\\%\\&\\_\\~\\^\\\\])");
     result = std::regex_replace(result, escapePattern, "\\$1");

     // Fix double escaping that might have occurred from other conversions
     result = std::regex_replace(result, std::regex("\\\\\\\\(textbf|textit|texttt|href|includegraphics|begin|end|item|section|subsection|subsubsection|paragraph|subparagraph|cite)"), "\\$1");
