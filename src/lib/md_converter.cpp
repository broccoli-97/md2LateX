...
std::string MarkdownConverter::convertLists(const std::string &line,
                                            bool &inList, int &listDepth) {
...
std::string MarkdownConverter::convertBlockquotes(const std::string &line,
                                                  bool &inQuote) {
...
std::string MarkdownConverter::convertCitations(const std::string &line) {
  ...
  while (
      std::regex_search(searchStart, result.cend(), matches, citationPattern)) {
...
//{
//     // First, we need to identify and protect LaTeX commands that are already
//     present
...
//         std::string placeholder = "##LATEXCMD" +
//         std::to_string(commands.size()) + "##"; commands.push_back(cmd);
//
...
std::string MarkdownConverter::escapeLatexChars(const std::string &text) {
  ...
  result = std::regex_replace(
      result,
      std::regex(
          "\\\\\\\\(textbf|textit|texttt|href|includegraphics|begin|end|item|"
