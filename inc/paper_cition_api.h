...
public:
  QueryResult query(const std::string &query_string) override {
    ...
    // 构建URL并对查询字符串进行URL编码
    char *encoded_query =
        curl_easy_escape(curl, query_string.c_str(), query_string.length());
    if (!encoded_query) {
    ...
    std::string url =
        "https://api.crossref.org/works?query=" + std::string(encoded_query) +
    ...
    try {
      ...
      for (const auto &item : items) {
        ...
        // 提取期刊信息
        if (item.contains("container-title") &&
            !item["container-title"].empty()) {
        ...
        if (item.contains("published") &&
            item["published"].contains("date-parts") &&
            !item["published"]["date-parts"].empty() &&
            !item["published"]["date-parts"][0].empty()) {
          paper.year =
              std::to_string(item["published"]["date-parts"][0][0].get<int>());
        }
        ...
        if (!paper.authors.empty() && !paper.year.empty()) {
          ...
          if (!paper.doi.empty()) {
            ...
            short_title.erase(std::remove_if(short_title.begin(),
                                             short_title.end(),
                                             [](unsigned char c) {
            ...
            paper.citation_key =
                "unknown" + std::to_string(result.papers.size() + 1);
          }
        }
...
public:
  QueryResult query(const std::string &query_string) override {
    ...
    std::string url =
        "https://scholar.google.com/scholar?q=" + std::string(encoded_query) +
        "&hl=en&as_sdt=0,5";
    curl_free(encoded_query);
        ...
        if (in_result) {
          ...
          if (line.find("</div>") != std::string::npos) {
            ...
            if (!current_paper.title.empty()) {
              // 生成引用键
              if (!current_paper.authors.empty() &&
                  !current_paper.year.empty()) {
...
public:
  ...
  bool saveBibFile(const std::vector<PaperInfo> &papers,
    ...
    try {
      ...
      bib_file.close();
      std::cout << "BibTeX file saved: "
                << std::filesystem::absolute(filename).string() << std::endl;
      return true;
    } catch (const std::exception &e) {
