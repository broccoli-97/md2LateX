...
//        curl_easy_setopt(curl, CURLOPT_USERAGENT, "PaperCitationTool/1.0 (mailto:user@example.com)");
//        
//        // 执行请求
...
//                    !item["published"]["date-parts"].empty() && !item["published"]["date-parts"][0].empty()) {
...
//                                              paper.authors[0].substr(0, comma_pos) : paper.authors[0];
//                    
//                    // 移除非字母数字字符
//                    first_author.erase(
//                        std::remove_if(first_author.begin(), first_author.end(), 
...
//                    if (!paper.doi.empty()) {
//                        paper.citation_key = "doi" + paper.doi.substr(paper.doi.find_last_of("/") + 1);
//                    } else if (!paper.title.empty()) {
//                        std::string short_title = paper.title.substr(0, 20);
//                        short_title.erase(
//                            std::remove_if(short_title.begin(), short_title.end(), 
//                                          [](unsigned char c) { return !std::isalnum(c); }),
//                            short_title.end());
...
//            std::cout << "Querying " << source->name() << "..." << std::endl;
//            auto query_result = source->query(query_string);
//            
//            if (query_result.success) {
//                results.insert(results.end(), query_result.papers.begin(), query_result.papers.end());
//            } else {
...
//        std::string bib_entry = "@article{" + paper.citation_key + ",\n";
//        
...
//    
//    // 生成并保存.bib文件
//    bool saveBibFile(const std::vector<PaperInfo>& papers, const std::string& filename = "references.bib") {
//        try {
//            std::ofstream bib_file;
//            bib_file.open(filename
...
//            
//            bib_file.close();
//            std::cout << "BibTeX file saved: " << std::filesystem::absolute(filename).string() << std::endl;
//            return true;
//        } catch (const std::exception& e) {
//            std::cerr << "Error saving BibTeX file: " << e.what() << std::endl;
//            return false;
//        }
//    }
//};
            ...
            try
            {
                ...
                for (const auto& item : items)
                {
                    ...
                    if (!paper.authors.empty() && !paper.year.empty())
                    {
                        // 提取第一作者姓氏
                        size_t comma_pos = paper.authors[0].find(",");
                        std::string first_author = (comma_pos != std::string::npos) ? paper.authors[0].substr(0, comma_pos) : paper.authors[0];
                        ...
                        if (line.find("</div>") != std::string::npos)
                        {
                            ...
                            if (!current_paper.title.empty())
                            {
                                ...
                                if (!current_paper.authors.empty() && !current_paper.year.empty())
                                {
                                    ...
                                    std::string first_author = (comma_pos != std::string::npos) ? current_paper.authors[0].substr(0, comma_pos) : current_paper.authors[0];
